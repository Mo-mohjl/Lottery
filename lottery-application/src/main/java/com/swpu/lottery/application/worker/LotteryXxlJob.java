package com.swpu.lottery.application.worker;
...
import com.swpu.lottery.domain.activity.service.stateflow.IStateHandler;
...
public class LotteryXxlJob {
    ...
    public void lotteryActivityStateJobHandler() throws Exception {
        ...
        List<ActivityVO> activityVOS = activityDeploy.scanToDoActivityList(10L);
        if (activityVOS.isEmpty()){
            logger.info("扫描活动状态 End 暂无符合需要扫描的活动列表");
            return;
        }
        while (!activityVOS.isEmpty()) {
            ...
            ActivityVO activityVO = activityVOS.get(activityVOS.size() - 1);
            activityVOS = activityDeploy.scanToDoActivityList(activityVO.getId());
        }
